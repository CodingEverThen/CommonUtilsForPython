...
"""
    ...
    @Author  : LXW
    @Site    : 
    @File    : common_monitorSystemInfoByPid.py
    @Software: PyCharm
...
class PidMonitor():
    def __init__(self, pids, res_path):
        """
        初始化
        :param pids: 进程号数组集合
        ...
        for pid in pids:
            ...
            fullPath = os.path.join(res_path, filename)
                ...
                for files in os.listdir(dir_path):
                    ...
                    if len(f_p) == 2 and f_p[1] == "pid" and os.path.isfile(os.path.join(dir_path, files)):
                        with open(os.path.join(dir_path, files), "rb+") as rf:
                            ...
                            full_res[f_p[0]] = rb
                            pids.append(int(rb))
        res_info = "{time} >>>>>> FILENAME->PID:{info}".format(time=getCurrentTime(), info=json.dumps(full_res))
        print(res_info)
        with open(os.path.join(res_path, "FilesNameToPids.log"), "ab+") as fuf:
            fuf.write(res_info+"\n")
        # 循环执行300次，每次间隔1秒
        for i in range(0, 300):
            PidMonitor(pids=pids, res_path=res_path).multiProcessingMonitorPid_main()
            time.sleep(1)
    except Exception as e:
        print(e)
        traceback.print_exc()
